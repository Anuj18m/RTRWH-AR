...
export class CalculationEngine {
  // Quality factor based on environment
  ...
  private getSeason(monthIndex: number): string {
    if (monthIndex >= 2 && monthIndex <= 4) return 'premonsoon';
    if (monthIndex >= 5 && monthIndex <= 8) return 'monsoon';
    if (monthIndex >= 9 && monthIndex <= 10) return 'postmonsoon';
    return 'winter';
  }

  // Get seasonal multipliers for demand
  ...
  // Find maximum storage needed during dry periods
  ...
  // Calculate extra water available during monsoon
  ...
  private calculateHouseholdDemand(userInput: UserInput): number {
    ...
    if (userInput.purpose === 'Irrigation') purposeFactor = 1.8;
    if (userInput.purpose === 'Industrial') purposeFactor = 2.3;
